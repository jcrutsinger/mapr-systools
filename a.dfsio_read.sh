...
# distributed file system.  This can be overridden via the -Dtest.build.data=/path/to/dir flag. 
# For convenience, you can modify the ROOTDIR variable in this script 
# 
#
#
# Test will create <n> files of size <fsize>.  Default to set is 4GB (4000).  this number should be sufficiently large to allow for a long running test, and outstrip local and storage level Caching
#
...
#Set the DFS directory had write to.  This must match what you had put for the DFSIO write test
...
basename=dfsio_read-$(date -Imin|cut -c-16)
mkdir ${basename}
logname=${basename}/${basename}.log

#Do the read test.  Note that this will bomb out if the write test wasn't performed first.
